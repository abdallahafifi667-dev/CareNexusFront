...
import {
  requestCreatePost,
  fetchCategories,
} from "../../../pages/Doctor/stores/postSlice";
...
const CreatePostModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Modal overlay check: if click is on the overlay (not the modal content), close it
      if (modalRef.current && event.target.className === "create-post-overlay") {
        onClose();
      }
    };
  ...
  return (
    <div className="create-post-overlay">
      <div
        className={`create-post-modal ${i18n.language === "ar" ? "rtl" : "ltr"}`}
      >
          ...
          <form onSubmit={handleSubmit} className="post-form">
            ...
            >
              <option value="" disabled>
                {t("posts.select_category", "Select Category...")}
              </option>
              {categories.map((cat) => (
                <option key={cat._id} value={cat.text}>
                  {cat.text}
                </option>
              ))}
            </select>
            ...
            <div className="post-textarea-container">
              <textarea
                placeholder={t(
                  "posts.what_do_you_want_to_talk_about",
              ...
              <div className="right-tools">
                <label className="comments-toggle">
                  ...
                  <span className="toggle-label">
                    {t("posts.allow_comments", "Allow Comments")}
                  </span>
                </label>
                ...
                <button
                  ...
                  disabled={
                    !text.trim() || !title.trim() || !category || isLoading
                  }
                >
                  {isLoading
