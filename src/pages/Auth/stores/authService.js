  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      const token =
        ...
        response.data.token;
      const refreshToken =
        response.headers["refresh-token"] || response.data.refreshToken;
      if (token) {
...
export const verifyResetCode = createAsyncThunk(
  ...
  async ({ email, code }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(
        "/forget-password/validate-reset-password-code",
        { email, code },
      );
      return response.data;
    } catch (error) {
...
export const verifyToken = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post("/users/validLogin", {});
      const token =
        response.headers["auth-token"] ||
        response.data.accessToken ||
        response.data.token;
...
export const getSignedUploadUrl = createAsyncThunk(
  "auth/getSignedUploadUrl",
  async ({ uploadType, userId }, { rejectWithValue }) => {
    try {
      // Backend uses 'guideDocument' not 'medicalDocument' — normalize
      const normalizedType =
        uploadType === "medicalDocument" ? "guideDocument" : uploadType;
      const response = await axiosInstance.post("/users/gcs/sign-upload", {
...
export const getVerificationStatus = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/users/verifyDocuments/status");
      const token =
        response.headers["auth-token"] ||
        response.data.accessToken ||
        response.data.token;
