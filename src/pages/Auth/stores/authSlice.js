  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action) => {
        ...
        const user = action.payload.user || action.payload;
        const token =
          action.payload.accessToken || action.payload.token || state.token;
        state.user = user;
        state.token = token;
        saveUser(user);
      })
      ...
      // Get Verification Status (KYC polling)
      .addCase(getVerificationStatus.pending, (state) => {
        state.error = null;
      })
      .addCase(getVerificationStatus.fulfilled, (state, action) => {
        const documentationComplete = action.payload?.documentationComplete;
        const freshToken = action.payload?.accessToken || action.payload?.token;
        if (freshToken) {
          state.token = freshToken;
          localStorage.setItem("auth-token", freshToken);
        }
        if (documentationComplete !== undefined && state.user) {
          state.user = { ...state.user, documentation: documentationComplete };
          saveUser(state.user);
        }
      })
...
export const { logout, clearError, resetPasswordPhase, setUser, updateUser } =
  authSlice.actions;
