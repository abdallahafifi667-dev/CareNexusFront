...
const OrderDetails = () => {
  ...
  const { currentOrder, loading, actionLoading } = useSelector(
    (state) => state.doctor,
  );
  ...
  useEffect(() => {
    if (currentOrder) {
      dispatch(
        setHeaderTitle(
          `${t("doctor.orders_management")} #${currentOrder._id.slice(-6).toUpperCase()}`,
        ),
      );
    }
  ...
  const handleAction = async (actionType) => {
    const result = await dispatch(
      handleOrderAction({ actionType, orderId: id }),
    );
  ...
  if (!currentOrder)
    return (
      <div className="order-not-found">
        {t("errors.failed_to_fetch_orders")}
      </div>
    );
          ...
          <div className="info-card patient-card">
            ...
            <div className="patient-info">
              ...
              <div className="text">
                ...
                <p>
                  <Mail size={14} /> {currentOrder.userId?.email || "N/A"}
                </p>
                <p>
                  <Phone size={14} /> {currentOrder.userId?.phone || "N/A"}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="service-details">
              <div className="detail-item">
                ...
                <span className="value">
                  {t(`medical_service.${currentOrder.serviceType}`)}
                </span>
              </div>
              <div className="detail-item">
                <span className="label">{t("doctor.proposed_price")}</span>
                <span className="value price">
                  {currentOrder.price} {t("common.currency")}
                </span>
              </div>
            </div>
          </div>
