...
const OrderCard = ({ order, isActive, onClick }) => {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case "open":
        return { label: t("order_status.open"), class: "open" };
      case "confirmed":
        return { label: t("order_status.confirmed"), class: "confirmed" };
      case "in_progress":
        return { label: t("order_status.in_progress"), class: "in-progress" };
      case "completed":
        return { label: t("order_status.completed"), class: "completed" };
      case "cancelled":
        return { label: t("order_status.cancelled"), class: "cancelled" };
      default:
      ...
      <div className="item-content">
        <div className="item-header">
          <h4 className="patient-name">
            {order.patient?.username || order.title}
          </h4>
